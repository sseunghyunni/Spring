...
public class BoardController extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws 
			...
			for(int i=0;i<items.size();i++) {
				...
				if(fileItem.isFormField()) {
					...
					if(fileItem.getSize()>0) {
						...
						String fileName= fileItem.getName().substring(idx+1); //위의 if구문안에 다음경로부터인 파일명을 불러오기위해서 idx(\\ or /) + 1 인덱스를 불러온다
