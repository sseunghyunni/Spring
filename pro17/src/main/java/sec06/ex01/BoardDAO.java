...
public class BoardDAO {
	 ...
	 public List<ArticleVO> selectAllArticles(){
		 ...
		 try {
			 ...
			 String query ="SELECT function_hierarchical() AS articleNO, parentNO, "
			 		+ "imageFileName, @level AS level,title,content,id,writeDate FROM (SELECT @start_with:=0, "
			 		+ "@articleNO:=@start_with, @level:=0) tbl JOIN t_board";
	 ...
	 public ArticleVO selectArticle(int articleNO) {
		 ...
		 try {
			 ...
			 String query= "select articleNO, parentNO, title,content,NVL(imageFileName,'null') "
			 		+ "as imageFileName, id , writeDate from t_board where articleNO =?";
			 System.out.println(query);
			 pstmt=conn.prepareStatement(query);
			 pstmt.setInt(1, articleNO);
			 ResultSet rs= pstmt.executeQuery();
